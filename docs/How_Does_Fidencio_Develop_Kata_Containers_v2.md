> This document describes a lightweight, repeatable local → image → registry → VM+K8s closed-loop workflow for developing and validating Kata Containers.
...
Change to `${HOME}/.kcli` dir, and edit `${HOME}/.kcli/profiles.yml`
...
Deployed on the machine using the [official documentation](https://docs.docker.com/engine/install/ubuntu/#install-using-the-convenience-script).
...
#### b. Update kata-deploy with your custom values
